...
contextSettings = xbmcaddon.Addon('context.penguin').getSetting('context.penguin.settings')
traktManager = xbmcaddon.Addon('context.penguin').getSetting('context.penguin.traktManager')
clearProviders = xbmcaddon.Addon('context.penguin').getSetting('context.penguin.clearProviders')
clearBookmark = xbmcaddon.Addon('context.penguin').getSetting('context.penguin.clearBookmark')
rescrape = xbmcaddon.Addon('context.penguin').getSetting('context.penguin.rescrape')
playFromHere = xbmcaddon.Addon('context.penguin').getSetting('context.penguin.playFromHere')
playAction = xbmcaddon.Addon('plugin.video.penguin').getSetting('hosts.mode')
autoPlay = 'true' if playAction == '2' else ''
cm_autoPlay = xbmcaddon.Addon('context.penguin').getSetting('context.penguin.autoPlay')
sourceSelect = xbmcaddon.Addon('context.penguin').getSetting('context.penguin.sourceSelect')
findSimilar = xbmcaddon.Addon('context.penguin').getSetting('context.penguin.findSimilar')
browseSeries = xbmcaddon.Addon('context.penguin').getSetting('context.penguin.browseSeries')
browseEpisodes = xbmcaddon.Addon('context.penguin').getSetting('context.penguin.browseEpisodes')

homeWindow.setProperty('context.penguin.settings', contextSettings)
homeWindow.setProperty('context.penguin.traktManager', traktManager)
homeWindow.setProperty('context.penguin.clearProviders', clearProviders)
homeWindow.setProperty('context.penguin.clearBookmark', clearBookmark)
homeWindow.setProperty('context.penguin.rescrape', rescrape)
homeWindow.setProperty('context.penguin.playFromHere', playFromHere)
homeWindow.setProperty('plugin.video.penguin.autoPlay', autoPlay)
homeWindow.setProperty('context.penguin.autoPlay', cm_autoPlay)
homeWindow.setProperty('context.penguin.sourceSelect', sourceSelect)
homeWindow.setProperty('context.penguin.findSimilar', findSimilar)
homeWindow.setProperty('context.penguin.browseSeries', browseSeries)
homeWindow.setProperty('context.penguin.browseEpisodes', browseEpisodes)
...
if action is None:
	...
	if cache._find_cache_version():
		run = 'true'
	if run == 'true':
		control.execute('RunPlugin(plugin://plugin.video.penguin/?action=cleanSettings)')
...
####################################################
	...
	for i in items:
		...
		url = 'plugin://plugin.video.penguin/?action=play&title=%s&year=%s&imdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s&meta=%s&select="2"' % (
...
elif action == 'contextpenguinSettings':
	control.openSettings('0.0', 'context.penguin')
...
####################################################
	...
	if params.get('opensettings') == 'true':
		control.openSettings(query, "plugin.video.penguin")
