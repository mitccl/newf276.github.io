...
def getpenguinVersion():
	return xbmcaddon.Addon('plugin.video.penguin').getAddonInfo('version')
...
def artPath():
	theme = appearance()
	return os.path.join(xbmcaddon.Addon('plugin.video.penguin').getAddonInfo('path'), 'resources', 'artwork', theme)
...
def artwork():
	if condVisibility('System.HasAddon(script.penguin.artwork)'):
		execute('RunPlugin(plugin://script.penguin.artwork)')
...
def metaFile():
	# if condVisibility('System.HasAddon(script.penguin.metadata)'):
		# return os.path.join(xbmcaddon.Addon('script.penguin.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
...
def clean_settings():
	...
	for addon_id in ('plugin.video.penguin', 'script.module.openscrapers'):
		try:
			removed_settings = []
			active_settings = []
