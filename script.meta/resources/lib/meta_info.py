...
def make_trailer(trailer_url):
    ...
    if match: return 'plugin://plugin.video.youtube/play/?video_id=%s' % match.group(1)
...
def get_movie_metadata(movie, genres_dict=None):
    ...
    try: info['genre'] = u' / '.join([x['name'] for x in movie['genres']])
        ...
        if genres_dict: info['genre'] = u' / '.join([genres_dict[x] for x in movie['genre_ids']])
        else: info['genre'] = ''
    return info
...
def get_trakt_movie_metadata(movie, genres_dict=None):
    ...
    if 'genres' in movie:
        if genres_dict: info['genre'] = ' / '.join([genres_dict[x] for x in movie['genres']])
    else: info['genre'] = ''
...
def get_tvshow_metadata_tvdb(tvdb_show, banners=True):
    ...
    if banners: info['poster'] = tvdb_show.get_poster(language='en')
...
def get_season_metadata_tvdb(show_metadata, season, banners=True):
    ...
    if banners: info['poster'] = season.get_poster(language='en')
    return info
...
def get_episode_metadata_tmdb(season_metadata, episode):
    ...
    if episode['still_path']: info['poster'] = u'https://image.tmdb.org/t/p/original%s' % episode['still_path']
    elif season_metadata['poster']: info['poster'] = u'https://image.tmdb.org/t/p/original%s' % season_metadata['poster']
    else: info['poster'] = ''
...
def item_images(type, tmdb_id=None, imdb_id=None, tvdb_id=None, name=None):
    ...
    if type == 'movie' and tmdb_id != None and tmdb_id != '': response = Movies(tmdb_id).info()
    elif type == 'tv' and tmdb_id != None and tmdb_id != '': response = TV(tmdb_id).info()
    elif type == 'tv' and tvdb_id != None and tvdb_id != '': response = Find(tvdb_id).info(external_source='tvdb_id')
    elif imdb_id != None and imdb_id != '': response = Find(imdb_id).info(external_source='imdb_id')
