...
class VideoPlayer(xbmc.Player):
    ...
    def get_episode_id(self, dbid, tvdb_id, season, episode):
        ...
        else: episodes = [ep for ep in episodes if plugin.id in ep['file'] and '/%s/' % tvdb_id in ep['file'].replace('\\', '/')]
    ...
    def onPlayBackEnded(self):
        ...
        if self.totalTime > 0 and self.currentTime / self.totalTime >= 0.75: self.mark_as_watched()
        self.reset()
