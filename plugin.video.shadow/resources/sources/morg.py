...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    if tv_movie=='tv':
        x=requests.get("https://movies.org/api/releases/tv/%s/%s00%s"%(imdb_id,season,episode_n),headers=base_header,timeout=10).json()
        
    else:
        
        x=requests.get("https://movies.org/api/releases/movie/"+(imdb_id),headers=base_header,timeout=10).json()
    ...
    for results in x:
            ...
            if 'gounlimited.to' in link_pre:
                ...
                y=requests.get(link_pre,headers=base_header,timeout=10).content
                ...
                for link_in in data:
                    ...
                    try:
                        try_head = requests.get(link_in,headers=base_header, stream=True,verify=False,timeout=3)
