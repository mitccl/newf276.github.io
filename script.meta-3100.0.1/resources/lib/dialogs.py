...
def notify(msg, title, delay, image, sound=False):
    if xbmc.getInfoLabel('Window.Property(xmlfile)') != 'VideoFullScreen.xml': return xbmcgui.Dialog().notification(heading=title, message=msg, time=delay, icon=image, sound=sound)
    else: pass