...
def get_trakt_ids(*args, **kwargs):
    try: return Trakt.find_trakt_ids(*args, **kwargs)
    except: return None
...
def on_play_video(players, params, trakt_ids=None):
    ...
    link = selection['path']
    if link.startswith('videodb://'): return xbmc.executebuiltin('ActivateWindow(10025,"%s")' % link)
    elif link.endswith('.strm'): return action_playmedia(link)
    else:
        if trakt_ids: properties.set_property('script.trakt.ids', json.dumps(trakt_ids))
        return link
    return None
    ...
    for command_group in player.commands:  
        ...
        for command in command_group:
            ...
            if link == 'movies' and player.media == 'movies':
                ...
                if video: command_group_results.append(video)
                ...
                if not video: video = tools.get_episode_from_library(parameters['tmdb'], parameters['season'], parameters['episode'])
                if video: command_group_results.append(video)
            elif not command.get('steps'): command_group_results.append({'label': player.title, 'path': text.urlencode_path(link), 'action': command.get('action', 'PLAY')})
            else:
                steps = [text.to_unicode(step) for step in command['steps']]
                files, dirs = lister.get(link, steps, parameters)
                if files: command_group_results += [{'label': f['label'], 'path': (f['path']), 'action': command.get('action', 'PLAY')} for f in files]
            if command_group_results: break
        results += command_group_results
    if results: return player.title, results