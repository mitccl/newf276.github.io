...
def list_dir(path):
    ...
    try: response = RPC.files.get_directory(media='files', directory=path, properties=['season','episode'])
    except:
    ...
    for item in response.get('files', []):
        if item.has_key('file') and item.has_key('filetype') and item.has_key('label'):
            if item['filetype'] == 'directory':
                for ext in ('.xsp', '.xml'):
                    if item['file'].endswith(ext) or item['file'].endswith(ext+'/'): continue
                dirs.append({'path':item['file'], 'label':item['label'], 'season': item.get('season')})
            else: files.append({'path':item['file'], 'label':item['label'], 'season': item.get('season'), 'episode': item.get('episode')})
...
class KeyboardMonitor(threading.Thread):
    ...
    def release_if_owner(self):
        with self.access_lock:
            if self.owner_thread is threading.current_thread(): self.release()

    def prep_search_str(self, text):
        t_text = text.to_unicode(text)
        for chr in t_text:
            if ord(chr) >= 1488 and ord(chr) <= 1514: return text.to_utf8(text[::-1])
        return text.to_utf8(text)

class Lister:
    IGNORE_CHARS = ('.', '%20')
    def __init__(self, preserve_viewid=None, stop_flag=None):
        if stop_flag is None: stop_flag = threading.Event()
    ...
    def get(self, path, guidance, parameters):
        ...
        for key, value in parameters.items():
            if isinstance(value, basestring):
                for c in IGNORE_CHARS: value = value.replace(c, ' ')
                parameters[key] = regex_escape(value)
        try: return self._browse_external(path, guidance, parameters, unescaped_parameters)
        finally: self._restore_viewid()

    def is_active(self):
        return not self.stop_flag.is_set()

    def stop(self):
        if not self.stop_flag.is_set(): self.stop_flag.set()
    ...
    def _has_match(item, pattern, parameters):
        ...
        if item.get('season'):
            ...
            if item_season == param_season: season_infolabel_match = True
        if pattern == '{season}' and season_infolabel_match: return True
        episode_infolabel_match = False
        if item.get('episode'):
            item_episode = str(item.get('episode'))
            param_episode = str(parameters.get('episode', ''))
            if item_episode == param_episode: episode_infolabel_match = True
        if pattern == '{episode}' and episode_infolabel_match: return True
        if pattern == '{season}x{episode}' and \
         season_infolabel_match and episode_infolabel_match: return True
        label = item['label']
        ...
        for c in IGNORE_CHARS: label = label.replace(c, ' ')
        ...
        if match is not None and match.end() == len(label):
            if first_season and not match.group('season') in ('1', '', '01', None): return False
    ...
    def _browse_external(self, path, guidance, parameters, unescaped_parameters, depth=0):
        ...
        for i, hint in enumerate(guidance):
            if self.stop_flag.isSet() or xbmc.abortRequested: return [],[]
            if not path: break
            if hint.startswith('keyboard:'): hint = u'@' + hint
            if hint.startswith('@keyboard:') and i != len(guidance) - 1:
                ...
                keyboard_hint = term
                continue
            try: _, dirs, files = cached_list_dir(path, keyboard_hint)
            except: break
            finally:
            ...
            path = None
            if hint.startswith('><'): hint = hint.strip('><')
            if hint == '@any':
                for dir in dirs:
                ...
                if matched_dirs: path = matched_dirs[0]['path']
                if i == len(guidance) - 1:
