...
def to_unicode(obj):
    if isinstance(obj, basestring):
        ...
        obj = copy.deepcopy(obj)
        for key, val in obj.items(): obj[key] = to_unicode(val)
    elif obj is not None and hasattr(obj, '__iter__'): obj = obj.__class__([to_unicode(x) for x in obj])
    else: pass
    return obj

def number_to_text(number_text):
    if not number_text.isnumeric(): return number_text
    if number_text == '' or None: return ''
    else:
        number_text = int(number_text)
        numbers = [
            u'zero', u'one', u'two', u'three', u'four', u'five', u'six', u'seven', u'eight', u'nine', u'ten', u'eleven',
            u'twelve', u'thirteen', u'fourteen', u'fifteen', u'sixteen', u'seventeen', u'eighteen', u'nineteen']
        if number_text < 20: return numbers[number_text]
        else: return ''

def text_to_number(text):
    if text.isnumeric(): return text
    if text == '' or None: return ''
    else:
        numbers = [
            u'zero', u'one', u'two', u'three', u'four', u'five', u'six', u'seven', u'eight', u'nine', u'ten', u'eleven',
            u'twelve', u'thirteen', u'fourteen', u'fifteen', u'sixteen', u'seventeen', u'eighteen', u'nineteen']
        ...
        if text.lower() in numwords: return numwords[text.lower()]
        else: return ''
...
def is_ascii(s):
    try:
        if isinstance(s, basestring): s.decode()
        return True
    ...
    if splitted_text[0] in dictionary:
        ...
        for action in splitted_text[1:]:
            ...
            if match:
                ...
                if action == 'ws': value = value.replace(' ', params[0])
                elif action == 'replace': value = value.replace(params[0], params[1])
                elif action == 'text_to_number': value = text_to_number(value)
                elif action == 'number_to_text': value = number_to_text(value)
                else: pass
        return value
    return None
...
def apply_parameters(text, parameters):
    while True:
        try: text = text.format(**parameters)
