...
def play_movie(tmdb_id):
    ...
    for lang in meta_players.get_needed_langs(players):
        if lang == 'en': tmdb_data = movie
        else: tmdb_data = Movies(tmdb_id).info(language=lang, append_to_response='external_ids,alternative_titles,credits,images,keywords,releases,videos,translations,similar,reviews,lists,rating')
    ...
    if link:
        ...
        play_base.action_play({'label': movie_info['title'],
                               'path': link,
                               'info': movie_info,
                               'is_playable': True,
                               'info_type': 'video',
...
def get_movie_parameters(movie):
    ...
    if 'slug' in trakt_ids:
        ...
        else: parameters['slug'] = parameters['title'].lower().replace('~','').replace('#','').replace('%','').replace('&','').replace('*','').replace('{','').replace('}','').replace('\\','').replace(':','').replace('<','').replace('>','').replace('?','').replace('/','').replace('+','').replace('|','').replace('"','').replace(' ','-').replace('--','-')
    return parameters