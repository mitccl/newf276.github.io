import os,csv,time,json,urllib,string,datetime,threading,collections,random,shutil,functools,urlparse,re,logging,sys
...
class _PersistentDictMixin(object):
    def __init__(self, filename, flag='c', mode=None, file_format='pickle'):
        ...
        if flag != 'n' and os.access(filename, os.R_OK):
            ...
            with fileobj: self.load(fileobj)

    def id_generator(self, size=6, chars=string.ascii_uppercase + string.digits):
        return ''.join(random.choice(chars) for _ in range(size))

    def sync(self):
        with self.lock: self._sync()
    ...
    def _sync(self):
        ...
        try: self.dump(fileobj)
        except Exception as e:
            os.remove(tempname)
            raise
        finally: fileobj.close()
        shutil.move(tempname, self.filename)
        if self.mode is not None: os.chmod(self.filename, self.mode)
    ...
    def dump(self, fileobj):
        if self.file_format == 'csv': csv.writer(fileobj).writerows(self.raw_dict().items())
        elif self.file_format == 'json': json.dump(self.raw_dict(), fileobj, separators=(',', ':'))
        elif self.file_format == 'pickle': pickle.dump(dict(self.raw_dict()), fileobj, 2)
        else: raise NotImplementedError('Unknown format: ' + repr(self.file_format))

    def load(self, fileobj):
        for loader in (pickle.load, json.load, csv.reader):
            fileobj.seek(0)
            try: return self.initial_update(loader(fileobj))
...
class TimedStorage(_Storage):
    ...
    def __setitem__(self, key, val, raw=False):
        if raw: self._items[key] = val
        else: self._items[key] = (val, time.time())
    ...
    def initial_update(self, mapping):
        for key, val in mapping.items():
            _, timestamp = val
            if not self.TTL or (datetime.datetime.utcnow() - datetime.datetime.utcfromtimestamp(timestamp) < self.TTL): self.__setitem__(key, val, raw=True)

class ListItem(object):
    def __init__(self, label=None, label2=None, icon=None, thumbnail=None, path=None):
        kwargs = {'label': label, 'label2': label2, 'iconImage': icon, 'thumbnailImage': thumbnail, 'path': path}
    ...
    def add_context_menu_items(self, items, replace_items=False):
        ...
        if replace_items: self._context_menu_items = []
    ...
    def from_dict(cls, label=None, label2=None, icon=None, thumbnail=None, path=None, selected=None, info=None, properties=None, context_menu=None,
        ...
        if selected is not None: listitem.select(selected)
        if info: listitem.set_info(info_type, info)
        if is_playable: listitem.set_is_playable(True)
        if is_folder: listitem.is_folder = True
        if properties:
            if hasattr(properties, 'items'): properties = properties.items()
            for key, val in properties: listitem.set_property(key, val)
        if stream_info:
            for stream_type, stream_values in stream_info.items(): listitem.add_stream_info(stream_type, stream_values)
        if context_menu: listitem.add_context_menu_items(context_menu, replace_context_menu)
        art = {}
        if poster:
            art['poster'] = poster
            if isTV:
        ...
        if thumbnail: art['thumb'] = thumbnail
        if art:
            try: listitem._listitem.setArt(art)
            except: pass
        return listitem
...
class XBMCMixin(object):
    ...
    def cached(self, TTL=60 * 24, cache=None):
        cachename = cache
        if cachename is None: cachename = self._function_cache_name
        if not hasattr(self, '_unsynced_storages'): self._unsynced_storages = {}
        ...
        def decorating_function(function):
            ...
            def wrapper(*args, **kwargs):
                ...
                if kwargs: key += (kwd_mark,) + tuple(sorted(kwargs.items()))
                try: result = storage[key]
                except KeyError:
    ...
    def get_storage_s(unsynced_storages, storage_path, name='main', file_format='pickle', TTL=None):
        ...
        if storage is None:
            with XBMCMixin._lock:
                storage = unsynced_storages.get(filename)
                if storage is None:
                    if TTL: TTL = datetime.timedelta(minutes=TTL)
                    try: storage = TimedStorage(filename, file_format, TTL)
                    except ValueError:
                        os.remove(filename)
    ...
    def get_string(self, stringid):
        ...
        if not hasattr(self, '_strings'): self._strings = {}
        if not stringid in self._strings: self._strings[stringid] = self.addon.getLocalizedString(stringid)
        return self._strings[stringid]

    def set_content(self, content):
        contents = [
            'files', 'movies', 'tvshows', 'seasons', 'episodes', 'genres', 'years', 'actors',  'playlists',
            'plugins', 'studios', 'directors', 'sets', 'tags', 'countries', 'roles', 'images', 'Addons']
    ...
    def get_setting(self, key, converter=None, choices=None):
        ...
        if converter is str: return value
        elif converter is unicode: return value.decode('utf-8')
        elif converter is bool: return value == 'true'
        elif converter is int: return int(value)
        elif isinstance(choices, (list, tuple)): return choices[int(value)]
        elif converter is None:
            try: return json.loads(value)
            except: return value
        else: raise TypeError('Acceptable converters are str, unicode, bool and int. Acceptable choices are instances of list  or tuple.')
    ...
    def get_view_mode_id(self, view_mode):
        ...
        if view_mode_ids: return view_mode_ids.get(xbmc.getSkinDir())
        return None
    ...
    def keyboard(self, default=None, heading=None, hidden=False):
        if heading is None: heading = self.addon.getAddonInfo('name')
    ...
    def notify(self, msg='', title=None, delay=5000, image=''):
        if title is None: title = self.addon.getAddonInfo('name')
    ...
    def _listitemify(self, item):
        ...
        if not hasattr(item, 'as_tuple'):
            if 'info_type' not in item.keys(): item['info_type'] = info_type
            item = ListItem.from_dict(**item)
        return item
    ...
    def _add_subtitles(self, subtitles):
        ...
        for _ in xrange(30):
            ...
            time.sleep(1)
        else: raise Exception('No video playing. Aborted after 30 seconds.')
        player.setSubtitles(subtitles)

    def set_resolved_url(self, item=None, subtitles=None):
        if self._end_of_directory: raise Exception('Current Kodi handle has been removed. Either set_resolved_url(), end_of_directory(), or finish() has already been called.')
        self._end_of_directory = True
        succeeded = True
        if item is None:
            item = {}
            succeeded = False
        if isinstance(item, basestring): item = {'path': item}
        item = self._listitemify(item)
        item.set_played(True)
        xbmcplugin.setResolvedUrl(self.handle, succeeded, item.as_xbmc_listitem())
        if subtitles: self._add_subtitles(subtitles)
        return [item]

    def play_video(self, item, player=None):
        try: item['info_type'] = 'video'
        except TypeError: pass
        item = self._listitemify(item)
        item.set_played(True)
        if player: _player = xbmc.Player(player)
        else: _player = xbmc.Player()
        _player.play(item.get_path(), item.as_xbmc_listitem())
        return [item]

    def play_audio(self, item, player=None):
        try: item['info_type'] = 'audio'
        except TypeError: pass
        item = self._listitemify(item)
        item.set_played(True)
        if player: _player = xbmc.Player(player)
    ...
    def add_sort_method(self, sort_method, label2_mask=None):
        try: sort_method = SortMethod.from_string(sort_method)
        except AttributeError: pass
        if label2_mask: xbmcplugin.addSortMethod(self.handle, sort_method, label2_mask)
        else: xbmcplugin.addSortMethod(self.handle, sort_method)
    ...
    def finish(self, items=None, sort_methods=None, succeeded=True, update_listing=False, cache_to_disc=True, view_mode=None):
        ...
        if sort_methods:
            for sort_method in sort_methods:
                if not isinstance(sort_method, basestring) and hasattr(sort_method, '__len__'): self.add_sort_method(*sort_method)
                else: self.add_sort_method(sort_method)
        if view_mode is not None:
            try: view_mode_id = int(view_mode)
            except ValueError: view_mode_id = self.get_view_mode_id(view_mode)
            if view_mode_id is not None: self.set_view_mode(view_mode_id)
...
def unpickle_args(items):
    ...
    if pickled is None: return items
    pickled_keys = pickled[0].split(',')
    ret = {}
    for key, vals in items.items():
        if key in pickled_keys: ret[key] = [pickle.loads(val) for val in vals]
        else: ret[key] = vals
    return ret
...
def pickle_dict(items):
    ...
    for key, val in items.items():
        if isinstance(val, basestring): ret[key] = val
        else:
            pickled_keys.append(key)
            ret[key] = pickle.dumps(val)
    if pickled_keys: ret['_pickled'] = ','.join(pickled_keys)
    return ret
...
def unpickle_dict(items):
    ...
    ret = {}
    for key, val in items.items():
        if key in pickled_keys: ret[key] = pickle.loads(val)
        else: ret[key] = val
    return ret
...
class UrlRule(object):
    def __init__(self, url_rule, view_func, name, options):
        ...
        rule = self._url_rule
        if rule != '/': rule = self._url_rule.rstrip('/') + '/?'
        p = rule.replace('<', '(?P<').replace('>', '>[^/]+?)')
        try: self._regex = re.compile('^' + p + '$')
        except re.error as e: raise ValueError('There was a problem creating this URL rule. Ensure you do not have any unpaired angle brackets: "<" or ">"')
    ...
    def match(self, path):
        ...
        if not m: raise NotFoundException
    ...
    def _make_path(self, items):
        for key, val in items.items():
            if not isinstance(val, basestring): raise TypeError('Value "%s" for key "%s" must be an instance of basestring' % (val, key))
            items[key] = urllib.quote_plus(val)
        try: path = self._url_format.format(**items)
        except AttributeError:
            path = self._url_format
            for key, val in items.items(): path = path.replace('{%s}' % key, val)
        return path
    ...
    def make_path_qs(self, items):
        for key, val in items.items():
            if isinstance(val, (int, long)): items[key] = str(val)
...
class Plugin(XBMCMixin):
    def __init__(self, name=None, addon_id=None, filepath=None, info_type=None):
        ...
        if addon_id: self._addon = xbmcaddon.Addon(id=addon_id)
        else: self._addon = xbmcaddon.Addon()
        ...
        if not self._info_type:
            types = {'video': 'video', 'audio': 'music', 'image': 'pictures'}
        ...
        if not os.path.isdir(self._storage_path): os.makedirs(self._storage_path)
    ...
    @property
    def request(self):
        if self._request is None: raise Exception('Please ensure that `plugin.run()` has been called before attempting to access the current request.')
    ...
    def _parse_request(self, url=None, handle=None):
        if url is None:
            url = sys.argv[0]
            if len(sys.argv) == 3: url += sys.argv[2]
        if handle is None: handle = sys.argv[1]
    ...
    def cached_route(self, url_rule, name=None, options=None, TTL=None, cache=None):
        ...
        if TTL: cache_decorator = self.cached(TTL, cache=cache)
        else: cache_decorator = self.cached(cache=cache)
    ...
    def add_url_rule(self, url_rule, view_func, name, options=None):
        ...
        if name in self._view_functions.keys(): self._view_functions[name] = None
        else: self._view_functions[name] = rule
        self._routes.append(rule)

    def url_for(self, endpoint, **items):
        try: rule = self._view_functions[endpoint]
        except KeyError:
            try: rule = (rule for rule in self._view_functions.values() if rule.view_func == endpoint).next()
            except StopIteration: raise NotFoundException("%s does not match any known patterns." % endpoint)
        if not rule: raise AmbiguousUrlException
        pathqs = rule.make_path_qs(items)
        return 'plugin://%s%s' % (self._addon_id, pathqs)

    def _dispatch(self, path):
        for rule in self._routes:
            try: view_func, items = rule.match(path)
            except NotFoundException: continue
            listitems = view_func(**items)
            if not self._end_of_directory and self.handle >= 0:
                if listitems is None: self.finish(succeeded=False)
                else: listitems = self.finish(listitems)
            return listitems
    ...
    def run(self, test=False):
        ...
        if hasattr(self, '_unsynced_storages'):
            for storage in self._unsynced_storages.values(): storage.close()
        return items
