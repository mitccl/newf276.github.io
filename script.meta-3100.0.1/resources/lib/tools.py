...
def scan_library(path=None):
    while not xbmc.abortRequested and (xbmc.getCondVisibility('Library.IsScanning') or xbmc.getCondVisibility('Window.IsActive(progressdialog)')): xbmc.sleep(1000)
...
def get_movie_from_library(imdbnumber):
    ...
    for movie in db_movies.get('movies', []):
        if movie['imdbnumber'] != imdbnumber: continue
        if movie['file'].endswith('.strm'): continue
...
def get_episode_from_library(imdbnumber, season, episode):
    ...
    for show in db_shows.get('tvshows', []):
        if show['imdbnumber'] != imdbnumber: continue
        ...
        for ep in db_episodes.get('episodes', []):
            if ep['episode'] != episode: continue
            if ep['file'].endswith('.strm'): continue
...
def add_source(source_name, source_path, source_content, source_thumbnail, type='video'):
    ...
    if not os.path.exists(xml_file):
        with open(xml_file, 'w') as f: f.write('''
<sources>
    <programs>
    ...
    if existing_source and existing_source != source_path and source_content != '': _remove_source_content(existing_source)
    if _add_source_xml(xml_file, source_name, source_path, source_thumbnail, type=type) and source_content != '': _set_source_content(source_content)
...
def _add_source_xml(xml_file, name, path, thumbnail, type='video'):
    ...
    for source in sources.findall('source'):
        ...
        if source.find('thumbnail') is not None: xml_thumbnail = source.find('thumbnail').text
        else: xml_thumbnail = ''
    ...
    if existing_source is not None:
        ...
        if xml_name == name and xml_path == path and xml_thumbnail == thumbnail: return False
        elif xml_name == name:
...
def _indent_xml(elem, level=0):
    ...
    if len(elem):
        ...
        for elem in elem: _indent_xml(elem, level+1)
        if not elem.tail or not elem.tail.strip(): elem.tail = i
    else:
...
def _get_source_attr(xml_file, name, attr, type='video'):
    ...
    for source in sources.findall('source'):
        xml_name = source.find('name').text
        if xml_name == name: return source.find(attr).text
    return None
